const defaultCoffee = {
  ...
  class CoffeeState {
    ...
    constructor (checkboxSelector) {
      ...
      document.querySelectorAll(checkboxSelector).forEach(e => e.onclick = () => this.changeCurrent())
    }
  
    changeCurrent() {
      const fields = Array.from(document.querySelectorAll(this.selector)) // get array of all inputs
                          .filter(e => e.checked)
                          .map(e => e.value) // HTMLInputElement -> ValueType
      const current = this.coffies.find(coffee => this._isEqual(fields, coffee.ingredients))
      if (current) {
        this._currentTransform(current, '#result')
        return 
      }
      this._currentTransform(defaultCoffee, '#result')
    }
  
    _currentTransform(obj, selector) {
      document.querySelector(selector).innerHTML = 
          `
            <img class="result-img" src="${obj.image}" />
            <p class="result-title">${obj.title}</p>
  ...
  const state = new CoffeeState('.choose > input')
